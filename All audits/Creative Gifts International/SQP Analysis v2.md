...
*Estimated using $28 avg price for Tier 1 (matches P0 listing price ~$24-30), $25 for Tier 2 (broader piggy bank average, lower-end included), and $35 for Tier 3 (balloon dog statues skew premium). Tier 2 cart-add and purchase numbers are 11-month averages from the absolute-numbers tool.*
...
Rates below are 12-month volume-weighted for Tier 1 and Tier 2 (3-month brand sample is too thin), and 3-month for Tier 3.
...
| Tier 2 | 0.013% (Blocker, cap ~24-27%) | 1.15% vs 1.31% (~Healthy) | ATC rate 16.8% vs 22.9% (Blocker, ~26% below industry); cart-to-purchase too thin (0/18 over 12 months) | **Impression Share + ATC** | Brand functionally does not show up. Listing under-converts the broader piggy bank shopper at click-to-cart. Defer scaling until Tier 1 listing fixes are live, then expand into Tier 2 |
...
- The market is ~32x bigger than Tier 1 in cart adds (~19,200/mo vs ~590/mo at the annual level), so it's where the long-term ceiling is.
- Annual brand totals: 9,292 impressions, 107 clicks, 18 carts, 0 purchases against industry totals of 70.3M / 924k / 211k / 51.6k. CTR is roughly in line with industry (1.15% vs 1.31%), so when the brand shows up the click-through is fine.
- ATC rate is the second blocker: 16.8% vs 22.9% industry. With 107 brand clicks across 12 months, this sample is large enough to score. The brand is ~26% below industry on click-to-cart, indicating the listing under-converts the broader piggy bank shopper (consistent with the Tier 1 finding that non-Gold variants do not convert).
- Cart-to-purchase remains too thin to score (0 brand purchases on 18 carts over 12 months). SQP also underreports purchases for low-volume brands, so I am not putting weight on the 0%.
- Sequencing: this tier is **not the first action**. The listing fixes that address Tier 1 CVR (variant pricing/imagery, decor-led title) should also lift Tier 2 ATC rate. Once Tier 1 stabilizes and Tier 2 ATC rate is in line with industry, scale into Tier 2 via manual keyword campaigns.
...
graph TD
    A["🔍 Impressions
    Share: 0.59% of ~12-15% max
    ⚠️ PRIMARY BLOCKER #1"] --> B["👆 Clicks
    CTR: 1.13% vs 1.27% industry
    ✅ Healthy"]
    B --> C["🛒 Add to Cart
    ATC Rate: 24.5% vs 23.1% industry
    ✅ Above industry"]
    C --> D["💰 Purchases
    Cart-to-Purchase: 7.7% vs 21.8% industry
    ⚠️ PRIMARY BLOCKER #2"]
...
- **P0 (Dog Balloon Animal Piggy Bank), Tier 2 ATC rate is 26% below industry at the annual level (16.8% vs 22.9%).** This is a real listing signal, not just sample noise. The same variant-pricing/imagery fixes that lift Tier 1 should also lift Tier 2 ATC rate, which broadens the impact of the listing work.
- **P0 (Dog Balloon Animal Piggy Bank), Tier 2 is a ~$480k/mo market and the brand has 0.013% impression share.** This is the biggest visible ceiling in the audit. It is not the first move, but the action plan should converge on Tier 2 once the listing is healthy.
